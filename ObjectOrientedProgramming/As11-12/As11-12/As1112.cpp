...
As1112::As1112(Controller* c,QWidget *parent)
	: ctrl(c),QMainWindow(parent)
{
	ui.setupUi(this);
	this->init();
	dmodel = new DogModel{ c->get_adoption() };
	//this->DogsListWidget->setModel(this->model);
	QSortFilterProxyModel* proxyModel =
		new QSortFilterProxyModel{};
	//proxyModel->setSourceModel(this->dmodel);
	//this->adoption->setSortingEnabled(true);
	//this->adoption->setModel(proxyModel);
	//this->adoption->resizeColumnsToContents();
