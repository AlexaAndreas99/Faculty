...
public:
	...
	*/
	bool update_dog_repo(const std::string& name, const std::string& breed,  const std::string& new_name, const std::string& new_photo);
	FileRepo* get_repo() const { return repo; }
	FileRepo* get_adoption() const { return adoption; }
	std::vector<Dog> filterByAge(std::string age);
	std::vector<Dog> filterByName(std::string str);
	std::vector<Dog> filterByBreedandAge(std::string str,int a);
